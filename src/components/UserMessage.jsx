	...
	return (
		<>
			{userId > 0 && message.sender.userId === userId ?
				<>
					<div className={admin ? "chat__dialog__message message--outgoing" : "chat__dialog__message message--outgoing"}>
						...
						{message.files.length > 0 && message.files.map((file, index) =>
							file.contentType.includes('image') ?
								<div className="user-message__img user-message__img--self" key={index}>
									<img onLoad={() => {if (update) {update()}}} src={file.uri} alt=""/>
								</div>
								:
								<div className="user-message__text-wrapper" key={index}>
									<div className="user-message__text">
										<a target="_blank" href={file.uri}>{file.originalName}</a>
									</div>
								</div>
						)}
						{(prev.time !== new Date(message.timestamp).getMinutes() || prev.id !== message.sender.userId) && <div className="message-time">
				...
				<>
					<div className="chat__dialog__message message--incoming">
						...
						{(prev.time !== new Date(message.timestamp).getMinutes() || prev.id !== message.sender.userId) && <div className="message-time">
							{new Date(message.timestamp).getHours()}:{(new Date(message.timestamp).getMinutes()).toString().padStart(2, '0')}
						</div>}
					</div>
				</>
			}
